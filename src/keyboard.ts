// custom version of @solid-primitives/keyboard 1.0.7
 ...
 * ```
 */
export const useCurrentlyHeldKey = /*#__PURE__*/ createSharedRoot<
  Accessor<string | null>
>(() => {
  const [keys] = useKeyDownList();
 ...
 * ```
 */
export const useKeyDownSequence = /*#__PURE__*/ createSharedRoot<
  Accessor<string[][]>
>(() => {
  const [keys] = useKeyDownList();
...
export function createShortcut(
  ...
  createEffect(
    on(
      sequence,
      requireReset ? handleSequenceWithReset : handleSequenceWithoutReset,
    ),
  );
}
